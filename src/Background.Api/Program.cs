        ...
        if (status is not null)
        {
            if (!Enum.TryParse<JobStatus>(status, ignoreCase: true, out var parsed))
                return Results.BadRequest(new
                    { error = $"Invalid status. Valid: {string.Join(", ", Enum.GetNames<JobStatus>())}" });
            statusFilter = parsed;
        }
...
app.MapPost("/prompts", async (
        ...
        CancellationToken ct) =>
    {
        if (request.TopP is < 0.1 or > 0.99)
        {
...
app.MapGet("/jobs/{id:guid}/artifacts/{fileName}", async (
        ...
        CancellationToken ct) =>
    {
        ...
        if (key is null)
            ...
            {
                error =
                    $"Unknown artifact: '{fileName}'. Valid: raw.json, preprocessed.md, prompt.md, response.json, processed.json"
            });
